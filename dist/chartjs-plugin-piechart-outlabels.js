...
function parseFont(value, height) {
    var size = helpers.valueOrDefault(value.size, chart_js.Chart.defaults.font.size);
    if (value.resizable) {
        size = adaptTextSizeToHeight(height, value.minSize, value.maxSize);
    }
    var font = {
        family: helpers.valueOrDefault(value.family, chart_js.Chart.defaults.font.family),
        lineHeight: helpers.toLineHeight(value.lineHeight, size),
        size: size,
        style: helpers.valueOrDefault(value.style, chart_js.Chart.defaults.font.style),
...
var classes = {
    OutLabel: function (chart, index, ctx, config, context) {
        ...
        var text = helpers.resolve([config.text, defaultConfig.text], context, index);
        ...
        this.init = function (text, lines) {
            ...
            // Init style
            this.style = {
                backgroundColor: helpers.resolve([config.backgroundColor, defaultConfig.backgroundColor, "black"], context, index),
                borderColor: helpers.resolve([config.borderColor, defaultConfig.borderColor, "black"], context, index),
                ...
                lineColor: helpers.resolve([config.lineColor, defaultConfig.lineColor, "black"], context, index),
            ...
            this.stretch = helpers.resolve([config.stretch, defaultConfig.stretch], context, index);
            this.horizontalStrechPad = helpers.resolve([config.horizontalStrechPad, defaultConfig.horizontalStrechPad], context, index);
...
chart_js.defaults.plugins.outlabels = defaultConfig;
